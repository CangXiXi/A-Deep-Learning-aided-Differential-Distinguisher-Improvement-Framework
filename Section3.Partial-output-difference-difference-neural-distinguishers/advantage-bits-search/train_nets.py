...
from tensorflow.keras.layers import Dense, Conv1D, Input, Reshape, Permute, Add, Flatten, \
    BatchNormalization, Activation
from tensorflow.keras.models import Model
from tensorflow.keras.regularizers import l2

from Github import speck as sp
...
def train_speck_distinguisher(num_epochs, num_rounds, depth,bit,bit_num,wdir):
    bit = np.concatenate((bit, [i + 16 for i in bit], [i + 32 for i in bit], [i + 48 for i in bit]))
    #create the network
...
if __name__ =="__main__":
    train_speck_distinguisher(num_epochs=20, num_rounds=6, depth=5,bit=[5,12,13,14,15,21,28,29,37,44,45,46,47,53,60,61],bit_num=8,wdir='./原始bit/')
    train_speck_distinguisher(num_epochs=20, num_rounds=6, depth=5,bit=[5,12,13,14,15,21,28,29,37,44,45,46,47,53,60,61],bit_num=8,wdir='./成对bit/')
